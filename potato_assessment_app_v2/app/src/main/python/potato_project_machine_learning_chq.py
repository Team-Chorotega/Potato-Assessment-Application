...
#-----------------------------------------------------------
# These libraries need to be imported in Chaquopy: numpy, opencv, imutils, PIL, skimage, tensorflow 2.1.0, keras 2.3.1
import numpy as np
import cv2 
import imutils
...
def ref_obj_analysis(coin_string, ref_obj_thresh, markers):
    ...
    # Implement watershed
    coin_markers = skimage.segmentation.watershed(-dist_transform, coin_markers)
...
def main(data, coin_string):
    ...
    #-----------------------------------------------------------
    """
    machine learning segmentation and then Watershed
    """
    SIZE_X = 256
    SIZE_Y = 256
    
    model_name = join(dirname(__file__), "potato_model_40epoch_colab.h5")
    ...
    prediction_image = prediction.reshape((SIZE_Y, SIZE_X))
    ...
    if(coin_string != "None"):
        pixels_per_cm, coin_box, coin_area = ref_obj_analysis(coin_string, ref_obj_thresh, markers)
